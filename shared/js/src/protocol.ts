...
// This MUST stay identical to shared/src/protocol.rs
...
export function encodeFrame(frame: Frame): Uint8Array {
  ...
  view.setUint32(19, frame.payload.length, false);
  buf.set(frame.payload, HEADER_SIZE);
  return buf;
}

export function decodeFrame(data: Uint8Array): Frame {
  if (data.length < HEADER_SIZE) throw new Error(`Frame too short: ${data.length}`);
  const view = new DataView(data.buffer, data.byteOffset, data.byteLength);
  const version = data[0];
  if (version !== PROTOCOL_VERSION) throw new Error(`Version mismatch: ${version}`);
  const opcode = data[1] as Opcode;
  const flags  = data[2];
  const roomId = data.slice(3, 19);
  const payloadLen = view.getUint32(19, false);
  if (data.length < HEADER_SIZE + payloadLen) throw new Error('Incomplete payload');
  const payload = data.slice(HEADER_SIZE, HEADER_SIZE + payloadLen);
  return { version, opcode, flags, roomId, payload };
}

export function makeFrame(opcode: Opcode, roomIdBytes: Uint8Array, payload = new Uint8Array(0)): Uint8Array {
  return encodeFrame({ version: PROTOCOL_VERSION, opcode, flags: 0, roomId: roomIdBytes, payload });
}
